<template>
  <div class="app-container">
    <div class="search-container">
      <div class="fr">
        <el-select v-model="search.area" clearable placeholder="请选择地区" style="width: 180px;">
          <el-option v-for="(item, key) in area" :key="key" :label="item" :value="item"></el-option>
        </el-select>
        <el-select v-model="search.age" clearable placeholder="请选择年龄" style="width: 180px;">
          <el-option v-for="(item, key) in age" :key="key" :label="item + '岁'" :value="item"></el-option>
        </el-select>
        <el-select v-model="search.height" clearable placeholder="请选择身高" style="width: 180px;">
          <el-option v-for="(item, key) in height" :key="key" :label="item + 'cm'" :value="item"></el-option>
        </el-select>
        <el-select v-model="search.education" clearable placeholder="请选择学历" style="width: 180px;">
          <el-option v-for="(item, key) in education" :key="key" :label="item" :value="item"></el-option>
        </el-select>
        <el-select v-model="search.marriage" clearable placeholder="请选择婚史" style="width: 180px;">
          <el-option v-for="(item, key) in marriage" :key="key" :label="item" :value="item"></el-option>
        </el-select>
        <el-select v-model="search.status" clearable placeholder="请选择状态" style="width: 180px;">
          <el-option
    ...
    >
      ...
      <el-table-column align="center" class-name="status-col" label="婚史" prop="marriage" width="80">
      ...
      <el-table-column align="center" label="生成时间" prop="createTime"></el-table-column>
      <el-table-column align="center" label="完成时间" prop="finishTime"></el-table-column>
      <el-table-column align="center" label="标签" prop="randListTag">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" label="地区" prop="area" sortable></el-table-column>
      <el-table-column align="center" label="工作地" prop="work_location" sortable></el-table-column>
      <el-table-column align="center" label="认证标志" prop="userIcon">
        <template slot-scope="scope">
          <div class="i" v-html="scope.row.userIcon"></div>
        </template>
      </el-table-column>
      <el-table-column align="center" label="身高" prop="height" sortable></el-table-column>
      <el-table-column align="center" label="realUid" prop="realUid" sortable width="95">
        <template slot-scope="scope">
          <span @click="toDetailById(scope.row.realUid)">{{ scope.row.realUid }}</span>
        </template>
      </el-table-column>
      <el-table-column align="center" label="年龄" prop="age" sortable></el-table-column>
      ...
      <el-table-column align="center" fixed="right" label="操作" prop="realUid" width="90px">
        <template slot-scope="scope">
  ...
  methods: {
    ...
    fetchData() {
      ...
      getList({
        ...
        this.list.map(v => {
          v.finishTime = v.finishTime ? new Date(v.finishTime).toLocaleString() : '------'
        })
...
<style>
  ...
  .gjhy {
    background: url(../../assets/icons/icon_services_16_6.jpg) no-repeat scroll top;
  }
